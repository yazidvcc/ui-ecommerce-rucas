...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full border-4 border-primary bg-surface-container-lowest p-8 flex flex-col gap-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
        
        <div className="text-center">
          <h2 className="text-5xl font-black tracking-tighter uppercase mb-2">SIGN IN</h2>
          <p className="text-text-muted font-bold text-sm tracking-widest uppercase">Premium Streetwear</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          <div className="flex flex-col gap-2">
            ...
            <input
              id="email"
              type="email"
              className="w-full p-4 border-2 border-primary bg-surface font-bold text-sm text-on-surface placeholder:text-text-muted focus:outline-none focus:border-on-surface transition-colors"
          ...
          <button 
            ...
            disabled={loading}
          >
            {loading ? <span className="w-6 h-6 border-4 border-on-primary border-t-transparent rounded-full animate-spin inline-block"></span> : 'SIGN IN'}
          </button>
        </form>
        ...
        <p className="text-center font-bold text-sm tracking-wider uppercase text-text-muted mt-2">
          ...
          <Link to="/register" className="text-primary hover:text-on-surface transition-colors underline underline-offset-4">
            Create one
          </Link>
        </p>
